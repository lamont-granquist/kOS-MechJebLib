...
namespace kOS.AddOns.kOSMechJebLib
{
    ...
    public class KOSMechJebLibAddon : Addon
    {
        ...
        private static StateVector StateVector(Vector r, Vector v) => new StateVector(r, v);

        private static OrbitalElements KeplerianValue(ScalarValue l, ScalarValue ecc, ScalarValue inc, ScalarValue lan, ScalarValue argp, ScalarValue nu) => new OrbitalElements(l/(1-ecc*ecc), ecc, inc, lan, argp, nu, l);

        private StateVector StateVectorAt(OrbitInfo oinfo, TimeStamp ut)
        {
            Orbit o = (Orbit) orbitField.GetValue(oinfo);
        ...
        {
            (V3 rf, V3 vf) = MechJebLib.TwoBody.Shepperd.Solve(mu, dt, xi.Position.ToVector3D().ToV3(), xi.Velocity.ToVector3D().ToV3());
        ...
        {
            (V3 rf, V3 vf) = MechJebLib.TwoBody.Shepperd.Solve(mu, dt, ri.ToVector3D().ToV3(), vi.ToVector3D().ToV3());
            return new StateVector(new Vector(rf.ToVector3d()), new Vector(vf.ToVector3d()));
        }

        private static LambertReturn Lambert(ScalarValue mu, Vector r1, Vector r2, ScalarValue dt, ScalarIntValue nrev)
        {
            (V3 vi, V3 vf) = MechJebLib.Lambert.Gooding.Solve(mu, r1.ToVector3D().ToV3(), V3.zero, r2.ToVector3D().ToV3(), dt, nrev, bypass: true);
        ...
        {
            (double eanom, double nu) = MechJebLib.Functions.Astro.AnomaliesFromMean(Deg2Rad(manom), ecc);
        ...
        {
            (double sma, double ecc, double inc, double lan, double argp, double nu, double l) = MechJebLib.Functions.Astro.KeplerianFromStateVectors(mu, x.Position.ToVector3D().ToV3(), x.Velocity.ToVector3D().ToV3());
        ...
        {
            (double sma, double ecc, double inc, double lan, double argp, double nu, double l) = MechJebLib.Functions.Astro.KeplerianFromStateVectors(mu, r.ToVector3D().ToV3(), v.ToVector3D().ToV3());
            return new OrbitalElements(sma, ecc, Rad2Deg(inc), Rad2Deg(lan), Rad2Deg(argp), Rad2Deg(nu), l);
        }

        private static StateVector StateVectorsFromKeplerian(ScalarValue mu, OrbitalElements elem)
        {
            (V3 r, V3 v) = MechJebLib.Functions.Astro.StateVectorsFromKeplerian(mu, elem.L, elem.Ecc, Deg2Rad(elem.Inc), Deg2Rad(elem.Lan), Deg2Rad(elem.Argp), Deg2Rad(elem.Nu));
        ...
        {
            (V3 r, V3 v) = MechJebLib.Functions.Astro.StateVectorsFromKeplerian(mu, l, ecc, Deg2Rad(inc), Deg2Rad(lan), Deg2Rad(argp), Deg2Rad(nu));
            return new StateVector(new Vector(r.ToVector3d()), new Vector(v.ToVector3d()));
        }
    }
}
